...
launch_videodev()
{
	...
	VIDEOSOURCEDECODED="${VSRC} !  ${DEPAY} ! ${DECODE} "
	OVERLAYS=" ffmpegcolorspace ! timeoverlay !  clockoverlay halign=right valign=bottom shaded-background=true time-format="%H:%M:%S"  !"
...
#	gst-launch-0.10  ${VIDEOSOURCEDECODED} ! queue  ${EXTRAEFFECTS} !  videoscale ! video/x-raw-yuv,width=${SCALEWIDTH},height=${SCALEHEIGHT} ! alpha ! videomixer name=mix ! ${OVERLAYS}   v4l2sink device=${VIDEODEV}  sync=false ${OVERLAYPIPE}
	gst-launch-0.10   videomixer name=mix sink_1::xpos=0 sink_1::ypos=0 sink_1::alpha=0.5 sink_1::zorder=3 sink_1::xpos=0 sink_0::ypos=0 sink_0::alpha=1 sink_0::zorder=2 ! ${OVERLAYS}   v4l2sink device=${VIDEODEV}  sync=false   ${VIDEOSOURCEDECODED} ! queue  ! ffmpegcolorspace  ${EXTRAEFFECTS} !  videoscale ! video/x-raw-yuv,width=${SCALEWIDTH},height=${SCALEHEIGHT} !   mix.sink_0 ${OVERLAYPIPE}
