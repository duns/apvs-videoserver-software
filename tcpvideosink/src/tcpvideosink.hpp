...
/**
 * @defgroup tcp_video_sink Gstreamer TCP video sink
 * @ingroup tcp_video_streamer
 * @brief Gstreamer TCP video sink module.
 */

/**
 * @file tcpvideosink.hpp
 * @ingroup tcp_video_sink
 * @brief Type declarations and global function definitions for TCP video streaming sink module.
 */

#ifndef TCPVIDEOSINK_HPP
#define TCPVIDEOSINK_HPP
	...
	pipeline_bus_handler( GstBus* bus, GstMessage* msg, gpointer data )
	{
		...
		case GST_MESSAGE_ERROR:
		{
			...
			else
			{
				...
				g_object_set( G_OBJECT( gst_bin_get_by_name( GST_BIN( pipeline ), "v4l2sink" ) )
					, "sync", false
					, NULL );
		...
		case GST_MESSAGE_EOS:
		{
			...
			//g_object_set( G_OBJECT( gst_bin_get_by_name( GST_BIN( pipeline ), "filesink" ) )
			//	, "location", "/home/workspace/streaming_output/xxx.ts"
			//	, NULL );
